...
# [DL-NOTE] `*` makes `label` keyword-only — prevents accidental positional use
# where a caller might pass a third string arg expecting something else.
# [DL-QUESTION] label="initial" default has no active caller — initialize_admin endpoint
# takes creds from the request body; "initial" path may be dead code or planned.
def write_initial_credentials(email: str, password: str, *, label: str = "initial") -> Path:
    ...
    # [DL-INSIGHT] O_TRUNC (not O_EXCL): lets reset-password overwrite an existing file
    # atomically at 0600, without a separate unlink step that would create a race window.
